...
{
    public class AddContributionForm : Form
    {
        public double Amount => double.TryParse(txtAmount.Text, out var v) ? v : 0;
        private TextBox txtAmount;

        public AddContributionForm(SavingsGoal goal)
        {
            BuildUI(goal);
        }

        private void BuildUI(SavingsGoal goal)
        {
            Text = $"Add Contribution — {goal.Name}";
            ...
            {
                Text = $"{goal.Name}  —  ₱{goal.Remaining:N2} left to reach ₱{goal.Target:N2}",
                ForeColor = Color.FromArgb(180, 190, 210),
                AutoSize = false, Dock = DockStyle.Fill, Height = 40
            });

            panel.Controls.Add(new Label { Text = "Contribution Amount (₱):", ForeColor = Color.FromArgb(160, 170, 190), AutoSize = true });
